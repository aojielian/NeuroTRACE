...
make_unique_ids <- function(x, prefix = "sample") {
  x <- as.character(x)
  x[is.na(x) | x == ""] <- paste0(prefix, "_", which(is.na(x) | x == ""))
...
detect_expression_and_meta <- function(gandal_rdata) {
  ...
  meta <- as.data.frame(get(meta_name, env))

  msg("Using expression:", expr_name, "dim", paste(dim(expr), collapse = " x "))
  msg("Using metadata:", meta_name, "dim", paste(dim(meta), collapse = " x "))

  # Goal: output expression as samples x genes, metadata rows aligned 1:1 with expression rows.
  # Never set rownames(meta) to a duplicated donor/subject column.
  aligned_by <- NA_character_

  # Case 1: expression rows already correspond to metadata rows.
  if (nrow(expr) == nrow(meta)) {
    aligned_by <- "row_order_expr_rows_meta_rows"
    sample_ids <- rownames(expr)
    if (is.null(sample_ids) || any(is.na(sample_ids)) || any(sample_ids == "")) {
      sample_ids <- rownames(meta)
    }
    if (is.null(sample_ids) || length(sample_ids) != nrow(expr)) {
      sample_ids <- paste0("sample_", seq_len(nrow(expr)))
    }
    sample_ids <- make_unique_ids(sample_ids, "sample")
    rownames(expr) <- sample_ids
    rownames(meta) <- sample_ids

  # Case 2: expression columns correspond to metadata rows, so transpose.
  } else if (ncol(expr) == nrow(meta)) {
    aligned_by <- "row_order_expr_cols_meta_rows_transposed"
    expr <- t(expr)
    sample_ids <- rownames(expr)
    if (is.null(sample_ids) || any(is.na(sample_ids)) || any(sample_ids == "")) {
      sample_ids <- rownames(meta)
    ...
    # Case 3: try exact matching with unique metadata identifiers.
    meta_candidates <- list()
    meta_candidates[["rownames_meta"]] <- rownames(meta)
    for (cc in colnames(meta)) {
      v <- as.character(meta[[cc]])
      if (length(unique(v[!is.na(v) & v != ""])) > 20) {
        meta_candidates[[cc]] <- v
      }
    }

    best <- data.frame(
      orientation = character(),
      meta_source = character(),
      overlap = integer(),
      duplicated_meta_ids = integer(),
      stringsAsFactors = FALSE
    )

    expr_row_ids <- rownames(expr)
    expr_col_ids <- colnames(expr)

    for (src in names(meta_candidates)) {
      ids <- meta_candidates[[src]]
      ids <- as.character(ids)

      if (!is.null(expr_row_ids)) {
        best <- rbind(best, data.frame(
          orientation = "expr_rows",
          meta_source = src,
          overlap = length(intersect(expr_row_ids, ids)),
          duplicated_meta_ids = sum(duplicated(ids)),
          stringsAsFactors = FALSE
        ))
      }

      if (!is.null(expr_col_ids)) {
        best <- rbind(best, data.frame(
          orientation = "expr_cols",
          meta_source = src,
          overlap = length(intersect(expr_col_ids, ids)),
          duplicated_meta_ids = sum(duplicated(ids)),
          stringsAsFactors = FALSE
        ))
      }
    ...
    best <- best[order(-best$overlap, best$duplicated_meta_ids), , drop = FALSE]
    msg("Best metadata/expression alignment candidates:")
    print(head(best, 10))

    if (nrow(best) == 0 || best$overlap[1] < 20) {
      stop("Cannot align expression and metadata by row order or sample identifiers.")
    }

    chosen <- best[1, ]
    aligned_by <- paste0(chosen$orientation, "_", chosen$meta_source)

    meta_ids <- meta_candidates[[chosen$meta_source]]
    meta_ids <- as.character(meta_ids)

    # Do not allow ambiguous duplicated metadata IDs for direct matching.
    if (any(duplicated(meta_ids))) {
      msg("WARNING: chosen metadata ID source has duplicates. Keeping first occurrence for direct sample alignment.")
      keep_first <- !duplicated(meta_ids)
      meta_match <- meta[keep_first, , drop = FALSE]
      meta_ids_match <- meta_ids[keep_first]
    } else {
      meta_match <- meta
      meta_ids_match <- meta_ids
    }

    if (chosen$orientation == "expr_cols") {
      expr <- t(expr)
    }

    common <- intersect(rownames(expr), meta_ids_match)
    expr <- expr[common, , drop = FALSE]
    meta <- meta_match[match(common, meta_ids_match), , drop = FALSE]

    sample_ids <- make_unique_ids(common, "sample")
    rownames(expr) <- sample_ids
    rownames(meta) <- sample_ids
  }

  msg("Expression/metadata aligned by:", aligned_by)
  msg("Aligned expression dim:", paste(dim(expr), collapse = " x "))
  msg("Aligned metadata dim:", paste(dim(meta), collapse = " x "))
  ...
  list(expr = expr, meta = meta, expr_name = expr_name, meta_name = meta_name, aligned_by = aligned_by)
}

choose_subject_column <- function(meta) {
  candidates <- c(
    "individual", "Individual", "individualID", "IndividualID",
    "subject", "Subject", "donor", "Donor",
    "brain_id", "BrainID", "brnum", "BrNum",
    "case_id", "CaseID", "IID", "id", "ID"
  )

  existing <- candidates[candidates %in% colnames(meta)]
  if (length(existing) == 0) return(NA_character_)

  best <- data.frame(column = character(), n_unique = integer(), n_dup = integer(), stringsAsFactors = FALSE)
  ...
  # Prefer columns with duplicates, enough unique subjects, and not almost unique sample IDs.
  best2 <- best[best$n_dup > 0 & best$n_unique >= 20, , drop = FALSE]
  if (nrow(best2) == 0) return(NA_character_)

  best2 <- best2[order(-best2$n_unique), , drop = FALSE]
  best2$column[1]
}

aggregate_to_subject_if_possible <- function(expr, meta) {
  subject_col <- choose_subject_column(meta)

  if (is.na(subject_col)) {
    msg("No repeated subject/donor column detected; using sample-level discovery bootstrap.")
    return(list(expr = expr, meta = meta, subject_col = NA_character_, aggregated = FALSE))
  }
  ...
  if (!any(duplicated(sid))) {
    msg("Subject column detected but no duplicates; using sample-level data:", subject_col)
    return(list(expr = expr, meta = meta, subject_col = subject_col, aggregated = FALSE))
  }

  msg("Aggregating expression to subject level using column:", subject_col)
  msg("Sample rows before aggregation:", nrow(expr), "unique subjects:", length(unique(sid)))
  ...
  rownames(meta_agg) <- groups

  msg("Subject-level expression dim:", paste(dim(expr_agg), collapse = " x "))
  return(list(expr = expr_agg, meta = meta_agg, subject_col = subject_col, aggregated = TRUE))
}
...
prepare_model_data <- function(expr, meta) {
  ...
  # Important for Gandal:
  # raw rows may be region/sample-level, with repeated donor IDs.
  # Aggregate to subject-level when repeated donor/subject IDs are detected.
  agg <- aggregate_to_subject_if_possible(expr, meta)
  expr <- agg$expr
  meta <- agg$meta

  # Recompute diagnosis after aggregation.
  meta$dx_bootstrap_tmp <- standardize_dx(meta[[dx_col]])
  meta$dx_binary <- ifelse(meta$dx_bootstrap_tmp == "ASD", 1, 0)
  meta$subject_aggregation_col <- agg$subject_col
  meta$subject_aggregated <- agg$aggregated
...
main <- function() {
  ...
  diag <- data.frame(
    ...
    aligned_by = dat0$aligned_by,
    subject_aggregated = unique(meta$subject_aggregated)[1],
    subject_aggregation_col = unique(meta$subject_aggregation_col)[1],
    n_samples = nrow(meta),

...
def discover_alignment_files(search_root, outdir):
    ...
    records = []
    for p in files:
        # Skip very large unrelated files if possible, but still inspect header.
        try:
            dfh = read_table(p, nrows=5)
    ...
    cand = pd.DataFrame(records).sort_values(["score", "path"], ascending=[False, True])
...
def main():
    ...
    if args.input_table:
        ...
        usable = cand[cand["score"] >= 8].copy()
        if usable.shape[0] == 0:
            fail_md = os.path.join(args.outdir, "06_step12C_FAILED_no_alignment_component_table.md")
            with open(fail_md, "w") as f:
                f.write("# Step12C failed: no alignment component table detected\n\n")
                f.write("Inspect `00_step12C_candidate_alignment_component_files.tsv` and rerun with `--input_table` pointing to the table that contains no-PC1, PC2-PC6, and signed stage-profile component columns.\n")
            log(f"No usable alignment component table found. Wrote {fail_md}")
            return
